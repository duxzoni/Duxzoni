...
public class JCalendar extends JComboBox {
	...
	private static final long serialVersionUID = -3195524656845215445L;
	...
	public JCalendar(boolean showActualDate) {
		...
		formattedTextField.setBorder(((JComponent) getEditor()
				.getEditorComponent()).getBorder());
		if (showActualDate) {
	...
	private class MotifDateComboBoxUI extends MotifComboBoxUI {
		...
		private static final long serialVersionUID = 6347284930529027590L;
	...
	private class DatePopup extends BasicComboPopup {
		...
		private static final long serialVersionUID = 7519247542246298261L;
		...
		public void show(Component invoker, int x, int y) {
			constructDatePopup();
			super.show(JCalendar.this, JCalendar.this.getWidth() - 206,
					JCalendar.this.getHeight());
		}
	...
	private class DatePanel extends javax.swing.JPanel {
		...
		private static final long serialVersionUID = -1657354595449361808L;
		...
		public DatePanel() {
			...
			nextMonth.addMouseListener(new java.awt.event.MouseAdapter() {
				...
				@Override
				public void mousePressed(java.awt.event.MouseEvent evt) {
					nextMonth.setBorder(BorderFactory
			...
			previousMonth.addMouseListener(new java.awt.event.MouseAdapter() {
				...
				@Override
				public void mousePressed(java.awt.event.MouseEvent evt) {
					previousMonth.setBorder(BorderFactory
							.createLoweredBevelBorder());
					if (SwingUtilities.isLeftMouseButton(evt)) {
			...
			previousYear.addMouseListener(new java.awt.event.MouseAdapter() {
				...
				@Override
				public void mousePressed(java.awt.event.MouseEvent evt) {
					previousYear.setBorder(BorderFactory
							.createLoweredBevelBorder());
					if (SwingUtilities.isLeftMouseButton(evt)) {
			...
			month = new String[] { "Janeiro", "Fevereiro", "Março", "Abril",
		...
		private void initComponents() {
			...
			previousYear.setBorder(javax.swing.BorderFactory
					.createRaisedBevelBorder());
			...
			previousMonth
					.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
			previousMonth.setBorder(javax.swing.BorderFactory
					.createRaisedBevelBorder());
			...
			nextMonth.setBorder(javax.swing.BorderFactory
					.createRaisedBevelBorder());
			...
			nextYear.setBorder(javax.swing.BorderFactory
					.createRaisedBevelBorder());
			...
			weekAndDaysPanel.setBorder(javax.swing.BorderFactory
					.createEtchedBorder());
		...
		private void initTimer(final boolean b1, final boolean b2,
				...
				if (i + 1 >= firstDayOfMonth && index <= maximumDayOfMonth) {
					...
					if (index == selectedDay || index == dayOfMonth) {
						...
						if (index == selectedDay) {
							...
							if (i % 7 != 0) {
								day[i].setForeground(UIManager.getDefaults()
						...
						if (day[i].getText()
								.equals(String.valueOf(selectedDay))) {
							day[i].setBackground(Color.GRAY);
							day[i].setForeground(Color.WHITE);
							selectedLabel = day[i];
						}
						...
						public void mouseReleased(MouseEvent evt) {
							if (SwingUtilities.isLeftMouseButton(evt)) {
								if (getMousePosition() != null) {
									...
									String monthText = df.format(gc
											.get(Calendar.MONTH) + 1);
									String yearText = String.valueOf(gc
											.get(Calendar.YEAR));
									formattedTextField.setValue(dayText + "/"
											+ monthText + "/" + yearText);
								}
		...
		private void setDaysAndFill() {
			...
			dateLabel.setText(String.valueOf(monthText + ", "
		...
		private void configureDayLabels(JLabel day) {
			...
			gc.set(Calendar.DAY_OF_MONTH,
					Integer.parseInt(selectedLabel.getText()));
			if (gc.get(Calendar.DAY_OF_WEEK) != Calendar.SUNDAY) {
				selectedLabel.setForeground(UIManager.getDefaults().getColor(
						"label.foreground"));
			} else {
		...
		public void prepare(boolean rollYear, boolean rollMonth, boolean up,
		...
		public void prepareDate() {
			if (hasValidDate()) {
				int d = Integer.parseInt(formattedTextField.getValue()
						.toString().substring(0, 2));
				int m = Integer.parseInt(formattedTextField.getValue()
						.toString().substring(3, 5)) - 1;
				int y = Integer.parseInt(formattedTextField.getValue()
	...
	private void setValue() {
		if (isValidDate(formattedTextField.getText())) {
			...
			formattedTextField
					.setText(formattedTextField.getValue().toString());
		}
	}
	...
	public void setEditable(boolean aFlag) {
		if (!aFlag) {
			throw new IllegalArgumentException(
					"no sense in setting editable mode to false");
		}
	}
	...
	public Object getSelectedItem() {
		try {
			...
			int d = Integer.parseInt(formattedTextField.getValue().toString()
					.substring(0, 2));
			int m = Integer.parseInt(formattedTextField.getValue().toString()
					.substring(3, 5)) - 1;
			int y = Integer.parseInt(formattedTextField.getValue().toString()
					.substring(6));
	...
	public void setSelectedItem(Object anObject) {
		if (anObject != null) {
			if (anObject instanceof GregorianCalendar) {
				String dayText = String.valueOf(((GregorianCalendar) anObject)
						.get(Calendar.DAY_OF_MONTH));
				String monthText = String
						.valueOf(((GregorianCalendar) anObject)
								.get(Calendar.MONTH));
				String yearText = String.valueOf(((GregorianCalendar) anObject)
						.get(Calendar.YEAR));
				formattedTextField.setValue(dayText + "/" + monthText + "/"
						+ yearText);
			} else {
				super.setSelectedItem(null);
